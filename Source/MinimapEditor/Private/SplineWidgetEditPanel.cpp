...
#include "Widgets/SplineBuilder.h"
...
void SSplineWidgetEditPanel::PaintSplinePoints(const FSlatePaintContext& InPaintContext) const
{
	...
	for (int32 i = 0; i < SplineRef.Points.Num(); ++i)
	{
		...
		FSlateDrawElement::MakeBox(
			InPaintContext.OutDrawElements,
			InPaintContext.LayerId,
			InPaintContext.AllotedGeometry.ToPaintGeometry(PointSize, FSlateLayoutTransform(KeyIconLocation)),
			KeyBrush,
			InPaintContext.DrawEffect,
...
void SSplineWidgetEditPanel::PaintSplineTangent(const FSlatePaintContext& InPaintContext, FSlateSplinePoint SplinePoint) const
{
	...
	FSlateDrawElement::MakeBox(
		InPaintContext.OutDrawElements,
		InPaintContext.LayerId,
		InPaintContext.AllotedGeometry.ToPaintGeometry(PointSize, FSlateLayoutTransform(ArriveTangentIconLocation)),
	...
	FSlateDrawElement::MakeRotatedBox(
		InPaintContext.OutDrawElements,
		InPaintContext.LayerId,
		InPaintContext.AllotedGeometry.ToPaintGeometry(ArrowImage->ImageSize * ZoomFactor, FSlateLayoutTransform(ArrowDrawLocation)),
		ArrowImage,
		InPaintContext.DrawEffect,
		AngleInRadians,
		TOptional<FVector2D>(),
